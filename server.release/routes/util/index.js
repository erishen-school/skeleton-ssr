...
var ssrParameter = _project["default"].ssrParameter;
...
var goRoute = function goRoute(controller, params) {
  ...
  handleRoute(router, _util["default"].firstUpperCase(controller), dataArr, params);
  return router;
};
/**
 *
...
var handleRoute = function handleRoute(router, controller, dataArr, params) {
  router.get('/', function (req, res) {
    ...
    if (handleDefaultPage(req, res, controller, renderObj, dataArr)) return; // 显示 Demo URLs

    var urlPrefix = controller.toLowerCase();
    var urls = [];

    _lodash["default"].each(dataArr, function (item, index) {
      if (item) {
        ...
        if (action) {
          if (demo) {
            _lodash["default"].each(demo, function (demoItem, demoIndex) {
...
var getRenderObj = function getRenderObj(req, controller, action, params) {
  ...
  var reactObj = getReactControllerAction(req, controller, action); //console.log('reactObj', reactObj);
  ...
  var renderObj = {
    ...
    bundleUtil: _bundleConfig["default"],
    ...
    isRelease: isRelease,
    links: [],
    // 单独页面可增加的 css 链接, 放在 webresource/build/livestream/css 目录下
    scripts: [],
    // 单独页面可增加的 script 链接, 放在 webresource/build/livestream/js 目录下
...
var handleDefaultPage = function handleDefaultPage(req, res, controller, renderObj, dataArr) {
  var suffix = ''; //console.log('handleDefaultPage', controller, dataArr);
  ...
  if (suffix != '') {
    var reactObj = getReactControllerAction(req, controller, suffix); //console.log('reactObj', reactObj);

    dataArr = _config["default"][reactObj.controller.toLowerCase()]; //console.log('dataArr', dataArr);
...
var getReactControllerAction = function getReactControllerAction(req, controller, action) {
  ...
  if (baseUrls != '' && baseUrls.length > 1) {
    ...
    if (newUrls.length > 0) {
      newController = _util["default"].firstUpperCase(newUrls[0]);
    }

    if (newUrls.length > 1) {
      newAction = _util["default"].firstUpperCase(newUrls[1]);
    }
...
var handleRes = function handleRes(req, res, controller, action, renderObj) {
  var originalUrl = req.originalUrl;
  var renderHtml = 'Common/Layout.html'; //console.log('handleRes', controller, action, originalUrl);
  ...
  if (renderObj) {
    ...
    if (originalUrl.indexOf(ssrParameter) != -1) {
      var newKey = _util["default"].firstLowerCase(newController) + newAction; //console.log('handleRes', newController, newAction, newKey);
      ...
      if (seoItem) {
        ...
        _render["default"].handleSSR(seoItem, _action, req, res, function () {
...
var handleController = function handleController(req, controller, action) {
  ...
  var result = {};
  var reactObj = getReactControllerAction(req, controller, action);
  var controllerAction = reactObj.controller + reactObj.action; //console.log('controllerAction', controllerAction);
  // CSR 可用， SSR 不可用
