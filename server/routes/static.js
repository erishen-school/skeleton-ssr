...
const Config = require(`../../client/config/static`).default.getConfig(wrapperDic, { isHybrid: true });
...
const renderWrite = function(req, res, route, callback){
    ...
    let html = getHtml();
    let queryRelease = req.query.release;
    ...
    res.render('React/Home.html', { html: html }, function(err, resHTML){
        if(!err){
            ...
            if(filename != ''){
                ...
                let filePathPrefix = path.join(__dirname, '../../public') + projectConfig.hybridDictionary + '/';
                ...
                if(filename.indexOf('/') != -1){
                    ...
                    if(filenameArrLen == 2){
                        ...
                        if (!fs.existsSync(filenameArr[0]))
                            fs.mkdirSync(filenameArr[0]);
                    }
                        ...
                        if (!fs.existsSync(filenameArr[1]))
                            fs.mkdirSync(filenameArr[1]);
                    }
